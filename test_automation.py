...
DEFAULT_EXCEL_CANDIDATES = [
    str(ROOT_DIR / "IT23242104_Test_cases.xlsx"),
...
def _read_output(is_chat: bool, output_locator) -> str:
    # Try all methods regardless of is_chat — some output textareas are read-only
    # so input_value() returns empty; inner_text/text_content work better.
...
def _collect_visible_text_fields(page):
    fields = []
    try:
        locator = page.locator("textarea, [contenteditable='true'], [role='textbox']")
        count = locator.count()
...
def _score_field_for_role(locator, role_tokens: list[str]) -> int:
    try:
    ...
    hay = " ".join([str(meta.get(k, "")) for k in ["placeholder", "ariaLabel", "name", "id", "className"]]).lower()
    score = 0
...
def _find_chat_locators(page, timeout_ms: int):
    ...
    while time.time() < deadline:
        ...
        try:
            visible = _collect_visible_text_fields(page)
            if len(visible) >= 2:
                input_tokens = ["english", "singlish", "input", "source"]
                output_tokens = ["sinhala", "output", "result", "translated"]
                scored = []
                for loc in visible:
                    in_score = _score_field_for_role(loc, input_tokens)
                    out_score = _score_field_for_role(loc, output_tokens)
                    scored.append((loc, in_score, out_score))

                input_candidate = max(scored, key=lambda x: x[1])[0] if scored else visible[0]
                output_candidate = max(scored, key=lambda x: x[2])[0] if scored else visible[1]

                if input_candidate and output_candidate:
                    action = page.get_by_role("button", name=re.compile(r"^Transliterate$", re.IGNORECASE)).first
                    return input_candidate, output_candidate, action
    ...
    try:
                meta = page.evaluate(
                        """() => Array.from(document.querySelectorAll('textarea, [contenteditable="true"], [role="textbox"]')).map(t => ({
                            tag: t.tagName,
